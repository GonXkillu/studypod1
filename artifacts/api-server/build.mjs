...
async function buildAll() {
  ...
  // Build 1: local dev server → dist/index.mjs
  await esbuild({
  ...
  // Build 2: Vercel serverless entry → api/index.mjs
  await esbuild({
  ...
  // Copy public HTML/CSS/JS files alongside the Vercel function → api/public/
  await cp(
  ...
  console.log("Build complete: dist/ (local) and api/ (Vercel)");
}
